...
static bool game_apply_round_scores(game_state_t *game) {
    ...
    for (reveal_index = 0;
         ...
         reveal_index++) {
        int owner_index =
            round_get_reveal_owner_at(&game->current_round, (size_t)reveal_index);
        ...
        vote_total = game->current_round.vote_totals[owner_index];
        if (vote_total <= 0) {
            continue;
        }
        ...
        game->players[title_writer_index].score += vote_total * GAME_VOTE_TITLE_POINTS;
        game->players[owner_index].score += vote_total * GAME_VOTE_ANSWER_POINTS;
    }
